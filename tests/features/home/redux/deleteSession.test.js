...
import {
  ...
  reducer,
} from 'src/features/home/redux/deleteSession';

const middlewares = [thunk];
const mockStore = configureMockStore(middlewares);

describe('home/redux/deleteSession', () => {
  afterEach(() => {
    nock.cleanAll();
  });
  ...
  it('dispatches success action when deleteSession succeeds', () => {
    ...
    return store.dispatch(deleteSession())
      .then(() => {
        ...
        expect(actions[0]).to.have.property('type', HOME_DELETE_SESSION_BEGIN);
        expect(actions[1]).to.have.property('type', HOME_DELETE_SESSION_SUCCESS);
      });
  });
  ...
  it('dispatches failure action when deleteSession fails', () => {
    ...
    return store.dispatch(deleteSession({ error: true }))
      .catch(() => {
        const actions = store.getActions();
        expect(actions[0]).to.have.property('type', HOME_DELETE_SESSION_BEGIN);
        expect(actions[1]).to.have.property('type', HOME_DELETE_SESSION_FAILURE);
  ...
  it('handles action type HOME_DELETE_SESSION_BEGIN correctly', () => {
  ...
  it('handles action type HOME_DELETE_SESSION_SUCCESS correctly', () => {
    const prevState = { deleteSessionPending: true };
    const state = reducer(
      prevState,
      { type: HOME_DELETE_SESSION_SUCCESS, data: {} }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.deleteSessionPending).to.be.false;
  });

  it('handles action type HOME_DELETE_SESSION_FAILURE correctly', () => {
    const prevState = { deleteSessionPending: true };
    const state = reducer(
      prevState,
      { type: HOME_DELETE_SESSION_FAILURE, data: { error: new Error('some error') } }
    );
    expect(state).to.not.equal(prevState); // should be immutable
    expect(state.deleteSessionPending).to.be.false;
  ...
  it('handles action type HOME_DELETE_SESSION_DISMISS_ERROR correctly', () => {
